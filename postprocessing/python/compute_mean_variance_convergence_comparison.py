...
def plot_convergence(basename_a, 
    ...
    if "_" not in latex_variables[variable]:
        ...
        variable_with_supscript_a = f"{latex_variables[variable][:-2]}_{{{latex_variables[variable][-1]}, \\mathrm{{{name_a}}}}}'
        variable_with_supscript_b = f"{latex_variables[variable][:-2]}_{{{latex_variables[variable][-1]}, \\mathrm{{{name_b}}}}}'
        plt.ylabel(f'Error ($\\lVert {latex_stat[stat]}({latex_variable_with_subscript_a[variable]}^{{N}})-{latex_stat[stat]}({latex_variable_with_subscript_b}^{{N/2}})\\rVert_{{L^1(D)}}$)')
