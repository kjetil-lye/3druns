...
do
	...
	do
		...
		python -m submit_command_on_daint \
		       ...
		       --command "$STRUCTURE_BIN_DIR/structure_standalone -i $SCRATCH/3druns/richtmeyermeshkov/stats/p${H//./_}/N${x}/rm_cuda_${t}.nc -o structure_fbm_cuda_t_${t}_p_${p} --samples ${x} --number-of-h  $(( ($x*32)/1024 )) --nx $x --ny $x --nz $x --platform cuda --p ${p}" \
		       "$@"
	    done
	done
    done
