...
if __name__ == '__main__':
    ...
    base_repo_dir = repo.working_dir

    run_script_path = os.path.join(base_repo_dir, 'tools/run_sarus_pwd.sh')
...
""")
    ...
    parser.add_argument('--alsvinn_version', default='0.5.4', type=str,
                        help="Version of alsvinn to use.")
...
srun sarus run --mpi --entrypoint="bash" --mount=type=bind,source=$SCRATCH,destination=$SCRATCH alsvinn/alsvinn_cuda:release-{args.alsvinn_version} {run_script_path} {configuration_path} --multi-sample {args.multi_sample} --multi-x {number_of_nodes_per_direction} --multi-y {number_of_nodes_per_direction} --multi-z {number_of_nodes_per_direction} {os.path.basename(configuration_file)}
"""
